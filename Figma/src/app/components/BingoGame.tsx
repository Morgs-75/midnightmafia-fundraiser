...
import { CasinoNotifications } from "./CasinoNotifications";
import { FloatingBadges } from "./FloatingBadges";
import { UrgencyPopups } from "./UrgencyPopups";
import { LiveViewers } from "./LiveViewers";
import { PulsatingStats } from "./PulsatingStats";
...
export function BingoGame() {
  ...
  const handleCheckout = () => {
    // Check if user has 10 numbers - show special upsell for 5 more at $75
    const count = selectedNumbers.length;
    if (count === 10) {
      setIsUpsellOpen(true);
    }
    // Check if user has 2 or 3 numbers - show upsell
  ...
  const handleUpsellAccept = () => {
    ...
    // Handle 10 numbers - add 10 more for $150 (20 total for $325)
    if (count === 10) {
      ...
      const additionalNumbers = availableNumbers.slice(0, 10);
      setSelectedNumbers((prev) => [
        ...prev,
        ...additionalNumbers,
      ]);
    }
    // Handle 2 or 3 numbers - reach 5 total
    else {
      const numbersToAdd = count === 2 ? 3 : 2;
      ...
      const additionalNumbers = availableNumbers.slice(
        0,
        numbersToAdd,
      );
      setSelectedNumbers((prev) => [
  ...
  const handleConfirmPurchase = (data: {
    ...
    // Add to supporters feed
    const newSupporters: SupporterEntry[] = selectedNumbers.map(
      (num) => ({
        number: num,
        displayName: finalDisplayName,
        message: data.message,
    ...
    setSupporters((prev) => [...newSupporters, ...prev]);
  ...
  const handleClaimBonus = (
    ...
    if (popupType === "flash" || popupType === "bonus") {
      // For flash/bonus deal: auto-select 10 numbers for $200
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-950 to-black text-white">
      ...
      />
      <LiveViewers />
      <PulsatingStats numbers={numbers} />
      ...
      <SupportersFeed supporters={supporters} />
